...
| Proposal | Outcome |
| --- | --- |
| `20260922_0010_reading_content_engine.py` | **OPEN.** Six tables behind the Admin Reading Content Engine — `reading_sources`, `reading_source_items`, `reading_articles`, `reading_article_targets`, `reading_review_events`, `reading_ingestion_jobs`. Additive; nothing existing is altered. Awaiting independent architecture review, then human schema/runtime authorization. Chains on `20260916_0009` (this lane's head); `codex/work` has its own `0010`–`0012`, so integration needs an Alembic merge or a rebase — recorded, not resolved. See `docs/project/READING_CONTENT_ENGINE_SCHEMA_REVIEW_REQUEST.md`. |
