...
| Proposal | Outcome |
| --- | --- |
| `20260924_0014_adaptive_reading.py` | Rewritten for D-075 (one canonical Reading flow and evidence model): the legacy generated-passage tables are archived read-only by rename, and `reading_attempts` becomes one canonical evidence table. The earlier `generated_session` design and its approval (`0d6efda`) are superseded. Independent review of this version **APPROVED** at `fdf198f` (round C1). Human schema/runtime authorization given 2026-09-24 (D-076) **for the admin sandbox only**, as a deliberate non-additive cutover deployed with the code that retires the generated flow; moved into `versions/`. Proved on SQLite and PostgreSQL by `tests/test_adaptive_reading_schema.py`. See `docs/project/ADAPTIVE_READING_ARCHITECTURE_REVIEW.md`. |
