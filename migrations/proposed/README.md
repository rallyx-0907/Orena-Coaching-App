...
| Proposal | Outcome |
| --- | --- |
| `20260924_0014_adaptive_reading.py` | **Open - not applied.** Rewritten for D-075 (one canonical Reading flow and evidence model): the legacy generated-passage tables are archived read-only by rename, and `reading_attempts` becomes one canonical evidence table. The earlier `generated_session` design and its approval (`0d6efda`) are superseded; this version is reviewed from the start. Two platform-content tables (`reading_comprehension_sets`, `reading_comprehension_questions`), one discardable learner projection (`reading_ability_projections`), the canonical `reading_attempts`, and `reading_articles.content_kind`. Proved on SQLite and PostgreSQL by `tests/test_adaptive_reading_schema_proposed.py`, which applies it from here without moving it. Moving it needs an APPROVED review and the human's schema/runtime authorization. See `docs/project/ADAPTIVE_READING_ARCHITECTURE_REVIEW.md`. |
