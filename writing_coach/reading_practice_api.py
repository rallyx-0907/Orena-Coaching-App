...
* `POST /sets/{id}/questions/{id}/grade`  immediate feedback on one answer,
                             without writing learner evidence
* `POST /attempts`           submit answers - idempotent by `operation_id`;
                             ...
                             server, only on a `recommendation` it issued from
                             `/next` for this account and set
* `GET  /evidence`           the learner's canonical attempts, newest first
* `GET  /ability`            the ability projection under the current policy

**Learner submit is off until the complete live E2E has passed (D-083).**
...
from writing_coach.persistence.reading_evidence_repository import ReadingEvidenceError, ReadingEvidenceRepository
...
class AttemptBody(BaseModel):
    ...
    # The recommendation `/next` issued, passed back untouched when the learner
    # came from it. The server verifies it; a body cannot state provenance
    # itself (`selection_policy_version` is refused by `extra="forbid"`).
    recommendation: str | None = Field(default=None, max_length=1024)


class QuestionChoiceBody(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
@router.post("/sets/{set_id}/questions/{question_id}/grade")
def grade_question(set_id: str, question_id: str, payload: QuestionChoiceBody,
                   response: Response) -> dict[str, Any]:
    _no_store(response)
    if not submit_enabled():
        raise orena_http_error(503, "reading_submit_disabled", "Reading practice is not available yet.")
    try:
        result = _guarded(lambda: _repo().grade_question(
            set_id, question_id, payload.selected_index, support_language=_support(),
        ))
    except ReadingEvidenceError as exc:
        raise orena_http_error(422, exc.code, str(exc)) from exc
    if result is None:
        raise orena_http_error(404, "reading_question_not_available", "This question is not available.")
