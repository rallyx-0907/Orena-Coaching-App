...
class ContextualGlossIn(BaseModel):
    """A word or short phrase the learner pointed at, and the sentence around it."""
...
def _gloss_schema() -> dict[str, Any]:
    return {
        ...
        "required": ["meaning", "base_form", "pronunciation", "part_of_speech"],
        "additionalProperties": False,
    }


def _local_word_facts(language: str, context: str, selection: str) -> dict[str, str]:
    ...
    about its use. Only a selection that is exactly one token gets a class; a
    phrase has none to give.
    """

    # Searched in the original text: casefolding can change a string's length,
    # and these offsets have to index the context the tagger reads.
    found = re.search(re.escape(selection), context, flags=re.IGNORECASE)
    if found is None:
        return {"part_of_speech": "", "pronunciation": "", "base_form": ""}
    start, end = found.span()
    tokens = [
        token
        for token in _annotate(language, context)
    ...
    return {
        ...
        "pronunciation": " ".join(t["pronunciation"] for t in tokens if t["pronunciation"]),
        "base_form": tokens[0]["lemma"] if single and language == "zh" else "",
    }


@contextual_router.post("/gloss")
def contextual_gloss(payload: ContextualGlossIn) -> dict[str, Any]:
    """The meaning a word has in the sentence the learner is reading - fast.

    A gloss is the first answer, not the whole explanation: one short meaning,
    the dictionary form and how it sounds. Anything deeper is the contextual
    explanation this sits beside. Word class and Chinese pinyin come from the
    shared local tagger, so they stay deterministic and survive a provider that
    is not answering.
    """
    ...
    source = payload.text.strip()
    context = payload.context.strip()
    if not source or source.casefold() not in context.casefold():
        raise HTTPException(422, "Selected text must come from the supplied learner context.")

    local = _local_word_facts(language, context, source)
    answer = {
        "available": False,
        "claim": "contextual_gloss_unavailable",
        "source_language": language,
        "target_language": target,
        "selected_text": source,
        "meaning": "",
    ...
    definition = support_language(target)
    target_name = definition.translation_label if definition else target
    source_name = "Simplified Chinese" if language == "zh" else "English"
    system = (
        f"You are a bilingual learner's dictionary. The learner is reading {source_name} "
        f"and needs the answer in {target_name}. Give only the meaning the selected "
        "words have in this exact sentence, not every sense they can have, in under "
        "twelve words. base_form is the dictionary form in the source language. "
        "pronunciation is IPA for English and tone-mark pinyin for Chinese. "
        "part_of_speech is one of: " + ", ".join(sorted(_ALLOWED_POS)) + ", or empty "
        "for a phrase. Never cite a source, dictionary or corpus you were not given."
    )
    user = f"SENTENCE:\n{context}\n\nSELECTED:\n{source}"
    try:
        raw = _run_structured(
    ...
    meaning = re.sub(r"\s+", " ", str(raw.get("meaning") or "")).strip()[:240]
    if not meaning:
        return answer
    model_pos = str(raw.get("part_of_speech") or "").strip().casefold()
    answer.update(
        {
            "available": True,
            "claim": "contextual_gloss",
            "meaning": meaning,
            "base_form": answer["base_form"]
            or str(raw.get("base_form") or "").strip()[:80],
            "part_of_speech": answer["part_of_speech"]
            or (model_pos if model_pos in _ALLOWED_POS else ""),
            "pronunciation": answer["pronunciation"]
            or str(raw.get("pronunciation") or "").strip()[:120],
        }
    )
    return answer
