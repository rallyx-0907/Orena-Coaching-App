...
`migrations/versions/20260924_0016_adaptive_reading.py` - authorized for the
admin sandbox by D-083 - applied to a throwaway database, with the rows each
...
- **SQLite**, always, including in CI: the schema is built the way the
  ...
  proved by the rehearsals: `downgrade()` to `20260924_0015`, legacy rows
...
- **PostgreSQL**, when `ORENA_TEST_POSTGRES_URL` names a throwaway database:
  the real chain `20260811_0001 -> 20260924_0016`, in a fresh schema of its
...
the canonical Reading model of D-082: legacy generated-passage tables archived
...
MIGRATION = ROOT / "migrations" / "versions" / "20260924_0016_adaptive_reading.py"
...
def db(request, tmp_path_factory):
    ...
    with _pg_schema() as schema:
        command.upgrade(_alembic(schema), "20260924_0016")
...
_last_test_time: datetime | None = None


def _now() -> datetime:
    # Windows clocks may return the same instant to two adjacent review
    # operations. The schema correctly requires a *new* decision moment.
    global _last_test_time
    current = datetime.now(UTC)
    if _last_test_time is not None and current <= _last_test_time:
        current = _last_test_time + timedelta(microseconds=1)
    _last_test_time = current
    return current
...
def test_the_downgrade_refusal_is_in_the_offline_script_too():
    ...
    command.upgrade(cfg, "20260924_0015:20260924_0016", sql=True)
    command.downgrade(cfg, "20260924_0016:20260924_0015", sql=True)
...
def _fresh(dialect: str, tmp_path: Path, seed=None):
    ...
    at `20260924_0015`, under the legacy names - what a real database holds when
    ...
    if dialect == "sqlite":
        ...
        # The hermetic head, taken down to 20260924_0015 by the migration's own
    ...
    with _pg_schema() as schema:
        cfg = _alembic(schema)
        engine = create_engine(_pg_schema_url(schema), future=True)
        command.upgrade(cfg, "20260924_0015")
        if seed:
            seed(engine)
        ...
        try:
            yield (
                engine,
                lambda: command.upgrade(cfg, "20260924_0016"),
                lambda: command.downgrade(cfg, "20260924_0015"),
            )
        finally:
            engine.dispose()
