...
function shell() {
  const c=ctx.c,current=ctx.location.page;
...
function preferences(onboarding=false) {
  ...
  sheet.querySelector('#preferencesForm').onsubmit=async event=>{
    ...
    const form=event.currentTarget,data=new FormData(form);form.inert=true;
    try {
      // Save full profile, preserving protected account settings. Language
      // changes wait for current evidence writes to finish.
      if(data.get('learning')!==ctx.language)await api.setLanguage(data.get('learning'));
      ctx.language=String(data.get('learning'));
      const prior=await api.learnerProfile();
      ctx.profile=await api.saveLearnerProfile({goal:prior.goal||'everyday',style:prior.style||'guided',pinyin:data.has('pinyin')?'auto':'off',native_language:data.get('support'),theme_preset:prior.theme_preset||'editorial'});
      ctx.support=ctx.profile.support_language||ctx.profile.native_language;
      ctx.ui=String(data.get('interface'));ctx.c=copy[ctx.ui];try{storage.setItem('orena.interface',ctx.ui);}catch{}
      ctx.memory=learnerMemory(storage,ctx.owner,ctx.language);sheet.close();
      ...
      history.replaceState(null,'',link());await render();
    }catch(error){sheet.querySelector('#preferenceError').textContent=error.message;form.inert=false;}
  };
}
function validVideo(value) {
  try{const url=new URL(value);if(url.protocol!=='https:'||url.username||url.password||url.port)return false;
    if(url.hostname==='youtu.be')return /^\/[\w-]{11}$/.test(url.pathname);
    return ['youtube.com','www.youtube.com','m.youtube.com'].includes(url.hostname)&&((url.pathname==='/watch'&&/^[\w-]{11}$/.test(url.searchParams.get('v')||''))||/^\/(shorts|embed)\/[\w-]{11}$/.test(url.pathname));
  }catch{return false;}
}
function importContent() {
  const c=ctx.c;
  const sheet=dialog({title:c.bring,body:`<div class="import-choice"><button data-kind="text" aria-pressed="true">${c.textTab}</button><button data-kind="media" aria-pressed="false">${c.mediaTab}</button></div><form id="textImport"><label>${c.title}<input name="title" maxlength="120" required></label><label>${c.text}<textarea name="text" rows="7" maxlength="12000" required lang="${ctx.language}"></textarea></label><p>${c.textHelp}</p><p role="alert"></p><button class="primary">${c.importAction} ↗</button></form><form id="mediaImport" hidden><label>${c.url}<input name="url" type="url" inputmode="url" required></label><p>${c.mediaHelp}</p><p role="alert"></p><button class="primary">${c.importAction} ↗</button></form>`});
  sheet.querySelectorAll('[data-kind]').forEach(button=>button.onclick=()=>{sheet.querySelectorAll('[data-kind]').forEach(x=>x.setAttribute('aria-pressed',String(x===button)));sheet.querySelector('#textImport').hidden=button.dataset.kind!=='text';sheet.querySelector('#mediaImport').hidden=button.dataset.kind!=='media';});
  sheet.querySelector('#textImport').onsubmit=event=>{event.preventDefault();try{const data=new FormData(event.currentTarget);const item=ctx.memory.add({title:data.get('title'),text:data.get('text')});sheet.close();ctx.go('encounter',{id:item.id});if(!ctx.memory.available)status(c.memoryUnavailable);}catch{event.currentTarget.querySelector('[role=alert]').textContent=c.invalidText;}};
  sheet.querySelector('#mediaImport').onsubmit=event=>{event.preventDefault();const url=String(new FormData(event.currentTarget).get('url')).trim();if(!validVideo(url)){event.currentTarget.querySelector('[role=alert]').textContent=c.invalidUrl;return;}sheet.close();ctx.go('encounter',{id:`url:${url}`});};
}
async function render() {
  const version=++generation;cleanup();cleanup=()=>{};
  document.querySelectorAll('dialog').forEach(x=>x.close());
  ctx.location=route(location.hash);ctx.alive=()=>generation===version;
  const scope={...ctx,alive:ctx.alive};shell();root.innerHTML=`<p class="loading" role="status">${ctx.c.loading}</p>`;
  window.scrollTo(0,0);
  try {
    const page=ctx.location.page;
    const result=page==='encounter'?await renderEncounter(root,scope):page==='expression'?await renderExpression(root,scope):page==='language'||ctx.location.intent==='recall'?await renderLanguage(root,scope):page==='practice'&&ctx.location.intent==='grammar'?await renderGrammar(root,scope):await renderWorld(root,scope);
    if(!scope.alive()){result?.();return;}
    cleanup=result||(()=>{});root.querySelector('h1')?.setAttribute('tabindex','-1');root.querySelector('h1')?.focus({preventScroll:true});
  }catch(error){if(scope.alive()){root.innerHTML=`<section class="empty"><h1>${ctx.c.unavailable}</h1><p>${esc(error.message)}</p><button class="primary" id="retry">${ctx.c.retry}</button></section>`;root.querySelector('#retry').onclick=render;}}
}
async function boot() {
  try {
    ...
    window.addEventListener('hashchange',render);await render();if(!profile.exists)preferences(true);
  }catch(error){root.innerHTML=`<section class="empty"><h1>${ctx.c.unavailable}</h1><p>${esc(error.message)}</p><button onclick="location.reload()">${ctx.c.retry}</button></section>`;}
}
boot();
