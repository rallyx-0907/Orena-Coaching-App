 ...
 *
 * The set is a **Thư viện của tôi collection** (D-074) - the learner's own
 * sets, which already exist, already carry one kind each, and already have a
 * server. Nothing here rebuilds a deck model beside them.
 *
...
function setRow(c, collection, { chosen = false, action = '' } = {}) {
  return `<button type="button" class="deck-row"${action ? ` ${action}` : ''} aria-pressed="${chosen}"${chosen ? ' data-chosen="true"' : ''}><span class="deck-row__art">${contentCover(
    { id: String(collection.id || collection.title || ''), title: collection.title || '', material: 'collection' },
  )}</span><span class="deck-row__title">${esc(collection.title)}</span>${icon(chosen ? 'check-circle' : 'circle', { size: 21, filled: chosen })}</button>`;
}

/* Frame 22: which of the learner's sets this word goes into. */
export function saveToDeckSheet(c, { word, note = '', collections = [], chosen = '' }) {
  const rows = collections
    .map((collection) =>
      setRow(c, collection, {
        chosen: String(collection.id) === String(chosen),
        action: `data-deck-pick="${esc(collection.id)}"`,
      }),
    )
    .join('');
  const into = collections.find((collection) => String(collection.id) === String(chosen));
...
export function addWordScreen(c, state) {
  const { word = '', meaning = '', example = '', collections = [], chosen = '', found = false, again = true, busy = false } = state;
  const into = collections.find((collection) => String(collection.id) === String(chosen));
  return `<section class="word-add"><header class="word-add__head"><button type="button" class="quiet" data-add-cancel>${esc(c.cancel)}</button><strong>${esc(c.addWord)}</strong><button type="button" class="quiet word-add__go" data-add-save${word.trim() && meaning.trim() && !busy ? '' : ' disabled'}>${esc(c.add)}</button></header><div class="word-add__body">${field(
    ...
    c.deck,
    `<button type="button" class="word-add__deck" data-add-pick-deck><span class="deck-row__art">${
      into
        ? contentCover({ id: String(into.id), title: into.title, material: 'collection' })
        : ''
    }</span><span>${esc(into ? into.title : c.chooseDeck)}</span>${icon('caret-up-down', { size: 17 })}</button>`,
...
export function createDeckScreen(c, state) {
  const { title = '', languages = [], language = '', locked = false, busy = false } = state;
  const options = languages
    ...
    .join('');
  return `<section class="deck-new"><header class="word-add__head"><button type="button" class="quiet" data-deck-cancel>${esc(c.cancel)}</button><strong>${esc(c.newDeck)}</strong><button type="button" class="quiet word-add__go" data-deck-create${title.trim() && language && !busy ? '' : ' disabled'}>${esc(c.create)}</button></header><div class="word-add__body"><div class="deck-new__cover">${contentCover(
    { id: String(title || 'new'), title: title || '', material: 'collection' },
  ...
  )}${field(c.deckLanguage, `<div class="deck-new__langs">${options}</div>${locked ? `<span class="word-add__note">${esc(c.deckLanguageLocked)}</span>` : ''}`)}</div></section>`;
}
