  ...
  async function openPractice(intent) {
    ...
    if (intent === 'dictation') {
      ...
      const paintHint = () => {
        ...
        hintPanel.innerHTML = `<small>${esc(c.hintTitle)}</small><p class="hint-slots" lang="${language}">${hint.slots.map((slot) => (slot.kind === 'structure' ? esc(slot.text) : `<span class="hint-word" data-kind="${slot.kind}"${slot.known ? ` data-known="${slot.known}"` : ''}>${[...slot.text].map((mark, index) => `<span class="hint-mark" data-state="${slot.kind === 'anchor' ? 'anchor' : index < (slot.known || 0) ? 'known' : 'unknown'}">${esc(mark)}</span>`).join('')}</span>`)).join('')}</p><p class="meta">${esc(hint.complete ? c.hintComplete : c.hintNote)}${hint.anchors ? ` · ${hint.anchors}/${hint.total} ${esc(c.hintAnchors)}` : ''}</p>`;
