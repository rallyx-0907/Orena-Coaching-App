...
/* Practice is where a learner arrives already knowing what they want to work
   on, so the four things they are most likely to be looking for lead, each
   holding the ways of working that belong to it. Dictation and Shadowing are
   ways of listening closely, not peers of Listening; presenting all eight as
   equal cards is how a learning product turns into a feature menu. Patterns &
   meaning and Recall sit apart because they are about language the learner has
   already met rather than a session to start. */
const practiceGroups = [
  ['follow', 'sound', ['follow', 'dictation', 'shadowing']],
  ['speaking', 'voice', ['speaking']],
  ['reading', 'book', ['reading']],
  ['writing', 'pen', ['writing']],
];
const practiceAside = [['grammar', 'spark'], ['recall', 'return']];
const practiceHref = (intent) =>
  intent === 'writing' ? link('expression') : link('practice', { intent });

export function practiceOverview(ctx) {
  const { c } = ctx, r = referenceCopy[ctx.ui];
  return `<section class="practice-workbench" aria-label="${esc(r.direct)}"><div class="workbench-note">${scene('focus',{size:'medium'})}<small>${esc(r.direct)}</small><p>${esc(c.practiceContext)}</p></div><div class="practice-ways">${practiceGroups.map(([lead,icon,ways])=>`<section class="practice-way"><a class="way-lead" href="${practiceHref(lead)}">${entryIcon(icon)}<div><h2>${esc(c[lead+'Name'])}</h2><p>${esc(c[lead+'Note'])}</p></div><span aria-hidden="true">↗</span></a>${ways.length>1?`<ul class="way-more">${ways.slice(1).map(w=>`<li><a href="${practiceHref(w)}"><strong>${esc(c[w+'Name'])}</strong><span>${esc(c[w+'Note'])}</span></a></li>`).join('')}</ul>`:''}</section>`).join('')}</div><section class="practice-aside" aria-label="${esc(r.keep)}"><ul>${practiceAside.map(([intent,icon])=>`<li><a href="${practiceHref(intent)}">${entryIcon(icon)}<div><strong>${esc(c[intent+'Name'])}</strong><span>${esc(c[intent+'Note'])}</span></div><span aria-hidden="true">↗</span></a></li>`).join('')}</ul></section></section>`;
}
...
export function discoverySpread(ctx, {media, text, catalogError}) {
  ...
  return `${editorialIntro(ctx,{title:r.invitation,note:r.welcome,state:'discovery',eyebrow:r.fieldNote})}
    ...
    <section class="story-walk"><div class="section-head"><div><small>${esc(r.fieldNote)}</small><h2>${esc(c.stories)}</h2></div><a class="quiet" href="${link('practice',{intent:'reading'})}">${esc(r.moreStories)} ↗</a></div>${text.slice(1).map((x,i)=>`<a class="story-path" href="${at(x.id)}"><span class="path-number" aria-hidden="true">0${i+1}</span><div><h3 lang="${language}">${esc(x.title)}</h3><p lang="${language}">${esc(x.subtitle)}</p><small>${esc(origin(x,c))}</small></div><span aria-hidden="true">↗</span></a>`).join('')}</section>
