...
export function discoverySpread(ctx, {media, text, catalogError, vocabularyFeed = ''}) {
  ...
  const mapItem = (href, icon, title, note) => `<a class="discover-map__item discover-map__item--primary" href="${href}">${entryIcon(icon)}<span><strong>${esc(title)}</strong><small>${esc(note)}</small></span><span aria-hidden="true">↗</span></a>`;
  const utilityItem = (href, icon, title) => `<a class="discover-map__utility" href="${href}">${entryIcon(icon)}<strong>${esc(title)}</strong><span aria-hidden="true">↗</span></a>`;
  const startMap = `<section class="discover-start" aria-labelledby="discover-start-title"><div class="discover-start__head"><div><small>${esc(r.browseAll)}</small><h2 id="discover-start-title">${esc(r.startHere)}</h2><p>${esc(r.startNote)}</p></div><a class="quiet" href="${link('continue')}">${esc(r.continue)} →</a></div><nav class="discover-map" aria-label="${esc(r.browseAll)}"><div class="discover-map__primary">${mapItem(link('practice',{intent:'reading'}),'book',r.goRead,r.goReadNote)}${mapItem(link('practice',{intent:'follow'}),'sound',r.goListen,r.goListenNote)}${mapItem(link('practice',{intent:'speaking'}),'voice',r.goSpeak,r.goSpeakNote)}</div><div class="discover-map__secondary">${utilityItem(link('practice'),'focus',r.practice)}${utilityItem(link('expression'),'pen',r.writing)}${utilityItem(link('language'),'leaf',c.vocabularyTitle)}</div></nav></section>`;
