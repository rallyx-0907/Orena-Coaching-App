...
export function renderSpeaking(root, ctx) {
  ...
  const picked =
    ...
    (source?.excerpt ? {key:location.id,title:source.title,prompt:source.excerpt,cue:c.voiceReflect,own:true} : null) || (!location.id ? invitations[0] : null);
  ...
  own.className='outline'; own.textContent=c.voiceOwn;
  root.querySelector('.voice-directions').append(own);
  own.onclick=()=>{
    const sheet=dialog({title:c.voiceOwn,body:`<form data-own-voice><label for="voiceOwnPrompt">${esc(c.voiceOwnNote)}</label><textarea id="voiceOwnPrompt" name="prompt" rows="4" maxlength="1200" required></textarea><button class="primary">${esc(c.voiceBegin)}</button></form>`});
    sheet.querySelector('[data-own-voice]').onsubmit=event=>{
      ...
      sheet.close();ctx.go('practice',{id,intent:'speaking'});
    };
  };
  if(picked.own)root.querySelector('.voice-directions > .meta').textContent=c.voiceOwnSource;
  memory.enter({
    id,
    title: picked.title,
    intent: 'speaking',
