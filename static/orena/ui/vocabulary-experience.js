...
export function renderVocabularyStudyCard(copy, card, { index = 0, practice = false } = {}) {
  const language = value(card.identity?.language) || 'en';
  const headword = value(card.headword);
  const pronunciation = value(card.pronunciation);
  ...
  const back = `<div class="vocabulary-study-card__back" data-study-back aria-hidden="true" inert><div class="vocabulary-study-card__back-body"><div class="vocabulary-study-card__headline"><h2 lang="${esc(language)}" data-vocabulary-deep="${esc(headword)}" data-study-no-flip role="button" tabindex="0" aria-label="${esc(copy.wordDeepOpen || headword)}">${esc(headword)}</h2>${vocabularyRankToken(copy, card)}${pronunciation ? `<span class="vocabulary-study-card__reading ds-data" lang="${esc(language)}" data-study-no-flip>${esc(pronunciation)}</span>` : ''}</div><p class="vocabulary-study__support" lang="${esc(copy.supportLanguage || 'en')}" data-study-no-flip>${esc(support)}</p>${definition && definition !== support ? `<p class="vocabulary-study__definition" lang="${esc(language)}" data-study-no-flip>${esc(definition)}</p>` : ''}${example ? `<section class="vocabulary-study-card__example"><p lang="${esc(exampleLang)}">${esc(example)}</p></section>` : ''}${card.usage ? `<section class="vocabulary-study__usage"><h3>${esc(copy.usage)}</h3><p lang="${esc(language)}">${esc(card.usage)}</p></section>` : ''}${orthography(card, copy)}</div><div class="vocabulary-study-card__footer">${practice ? '' : saved ? `<div class="vocabulary-study-card__grades" aria-label="${esc(copy.review)}"><button class="outline" data-study-grade="again">${icon('x', { size: 14 })}<span>${esc(copy.again)}</span></button><button class="primary" data-study-grade="got_it">${icon('check', { size: 14 })}<span>${esc(copy.know || copy.gotIt)}</span></button></div>` : `<button class="outline vocabulary-study-card__save" data-vocabulary-save="${esc(index)}" aria-label="${esc(saveLabel)}">${icon('plus', { size: 14 })}<span>${esc(copy.save)}</span></button>`}</div></div>`;
