...
function metadata(copy, card) {
  ...
  const framework = value(card.framework);
...
export function renderVocabularyCollectionCard(copy, collection, { index = 0 } = {}) {
  ...
  return `<article class="vocabulary-collection-card" data-vocabulary-collection-card="${esc(index)}"><button class="vocabulary-collection-card__open" data-vocabulary-collection="${esc(collection.id)}" data-open-collection="${esc(collection.id)}"><span class="vocabulary-collection-card__eyebrow">${esc(collection.level || '')} · ${esc(collection.framework || '')}</span><h3>${esc(collection.title || '')}</h3>${collection.topic ? `<p>${esc(collection.topic)}</p>` : ''}<div class="vocabulary-collection-card__progress"><span>${esc(learned)} / ${esc(total)} ${esc(copy.words || 'words')}</span><span>${esc(percent)}%</span></div><div class="vocabulary-progress" aria-hidden="true"><span style="width:${percent}%"></span></div><div class="vocabulary-collection-card__stats"><span>${esc(progress.learning_count || 0)} ${esc(copy.learning || 'Learning')}</span><span>${esc(progress.due_count || 0)} ${esc(copy.due || 'Due')}</span><span>${esc(progress.mastered_count || 0)} ${esc(copy.mastered || 'Mastered')}</span></div></button></article>`;
}
...
export function renderVocabularyFeedPreview(copy, card, { index = 0, saveAttribute = 'data-vocabulary-save' } = {}) {
  ...
  return `<article class="vocabulary-feed-item" data-vocabulary-feed-item="${esc(index)}"><div><span class="vocabulary-feed-item__level">${esc(value(card.level) || '—')} · ${esc(rankForVocabulary(card))}</span><h3 lang="${esc(language)}">${esc(card.headword)}</h3><p lang="${esc(copy.supportLanguage || 'en')}">${esc(meaning)}</p></div><div class="vocabulary-feed-item__actions"><button class="quiet" data-vocabulary-study="${esc(index)}">${esc(copy.study || copy.open)}</button><button class="quiet" ${saveAttribute}="${esc(index)}" aria-label="${esc(card.saved ? copy.saved : copy.save)}" ${card.saved ? 'disabled aria-pressed="true"' : ''}>${esc(card.saved ? copy.saved : copy.save)}</button></div></article>`;
}
