...
function mediaItem(item, intent, c) {
  return `<a class="voice-row" href="${link('encounter', { id: item.id, intent })}"><span class="voice-play" aria-hidden="true">▶</span><span><small>${mediaLine(item)}</small><strong lang="${item.language}">${esc(item.title)}</strong><span class="byline">${esc(item.source?.creator || origin(item, c))}</span></span><span aria-hidden="true">↗</span></a>`;
}
...
export async function renderWorld(root, ctx) {
  ...
  const headline = (title, note, kicker = '') =>
    `<div class="page-intro"><div><small>${esc(kicker)}</small><h1>${esc(title)}</h1><p>${esc(note)}</p></div></div>`;
  const recent = memory.value.continuation;
  const continuation = recent.length
    ? `<section class="continuation"><div><small>${c.continue}</small><p>${c.continueNote}</p></div><div>${recent
        .slice(0, 4)
        .map(
          (x) =>
            `<a href="${continuationLink(x)}"><span>${esc(x.title)}</span> →</a>`,
        )
        .join('')}</div></section>`
    : '';
