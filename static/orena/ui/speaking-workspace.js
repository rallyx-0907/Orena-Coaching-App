...
export function detailHtml({ s, c, word, language }) {
  ...
  const sounds = units.length
    ...
    : '';
  const said = fill(s.detailSaid, {
    verdict: word.flagged ? errorLabel(s, word.errorType) : s.passed,
...
<p class="sp-line" lang="${esc(language)}" data-sp-line>${sentenceHtml(line.text, language, null)}</p>${line.reading ? `<p class="sp-reading">${esc(line.reading)}</p>` : ''}${line.meaning ? `<p class="sp-meaning">${esc(line.meaning)}</p>` : ''}<p class="sp-hint" data-sp-hint role="status">${esc(s[`tapWord_${language === 'zh' ? 'zh' : 'en'}`])}</p>
