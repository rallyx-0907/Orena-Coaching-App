...
export function openDrawer({ title, body = '', label = 'Close', onClose } = {}) {
  const opener = document.activeElement;
  const element = document.createElement('dialog');
  element.className = 'ac-drawer';
  element.setAttribute('aria-label', title);
  element.innerHTML = `<div class="ac-drawer__head"><h2>${esc(title)}</h2><button type="button" class="ac-button" data-drawer-close>${esc(label)}</button></div><div class="ac-drawer__body" data-drawer-body>${body}</div>`;
