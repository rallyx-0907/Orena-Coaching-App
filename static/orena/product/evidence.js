...
export function dictationEvidence({asset,segment,language,previous={}}) {
  ...
  return {
    ...
    compare(answer){const result=evaluateListeningReconstruction({source_language:language,expected:segment.spoken_text||segment.original_text,answer});evidence={...evidence,presentation:'checked',checked_attempt_count:Math.min(1000,evidence.checked_attempt_count+1),best_accuracy_percent:Math.max(evidence.best_accuracy_percent??0,result.accuracy_percent),best_exact:evidence.best_exact||result.exact,last_answer:answer};return {result,diff:listeningReconstructionDiff({source_language:language,expected:segment.spoken_text||segment.original_text,answer})};},
