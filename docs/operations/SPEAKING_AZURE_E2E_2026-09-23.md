...
decisions it informs are the human's (D-074, and the SpeechSuper question below). Change when the
...
rather than neutral). Azure returns no pitch contour. So, under D-074 ("passed" is the provider's
