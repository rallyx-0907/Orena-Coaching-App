"""The canonical Reading cutover (D-082, D-083) - sandbox only after review.

The Admin-lane predecessor was authorized for its sandbox. The integration
revision `20260924_0016` needs independent delta review and explicit runtime
authorization before this apply command is used on a shared runtime.
...
`apply` is the migration itself, `20260924_0016`, non-additive. It is one
...
the inventory and decided it is test or development data (D-083): it refuses
...
def refusal(*, url: str, app_url: str, environ: dict[str, str], confirmed: str = "") -> str | None:
    ...
    if (parsed.hostname or "").casefold() == PRODUCTION_HOST or database_name(url) == PRODUCTION_DATABASE:
        return (f"that is the production runtime's database ({PRODUCTION_HOST}/{PRODUCTION_DATABASE}, the "
                "compose default): this authorization is for the admin sandbox only (D-083)")
    if not confirmed:
...
def apply(url: str, *, confirmed: str, expect_cluster: str, from_revision: str,
    ...
    module docstring)."""
    # One step only: 20260924_0015 -> 20260924_0016. A database at any other
...
def inventory(url: str) -> int:
    ...
    print("  The decision is the human's (D-083). To reset test data only after reporting this:")
...
def reset_legacy(url: str, *, expect_sessions: int, expect_attempts: int) -> int:
    ...
    with engine.begin() as connection:
        if _names(_tables(connection)) != ("reading_legacy_sessions", "reading_legacy_attempts"):
            print("refused: the legacy archive exists only after 20260924_0016 is applied")
            return 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--confirm-sandbox", default="", metavar="DATABASE",
                        help="required: the admin sandbox's database name (D-083)")
